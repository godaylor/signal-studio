export function validateProductionEnvironment(env) {
  ...
  if (env.SIGNAL_STUDIO_SERVERLESS === '1') {
    if (!['supabase', 'postgres'].includes(env.EXPORT_STORAGE_BACKEND)) throw new Error('Serverless exports require durable Supabase or PostgreSQL storage.');
    const storage = new URL(env.SUPABASE_URL || 'https://invalid.local');
    if (env.EXPORT_STORAGE_BACKEND === 'supabase' && (!env.SUPABASE_URL || storage.protocol !== 'https:' || storage.pathname !== '/' || storage.username || storage.password || storage.search || storage.hash || !env.SUPABASE_SERVICE_ROLE_KEY))
